...
export class RouteDetailComponent implements OnInit {
  ...
  async loadRouteDetail() {
    ...
    try {
      ...
      // let expedienteDetalleLocal: ExpedienteDetalleLocal | null = null;
      // for (const exp of expedientesLocal) {
      //   const response = await this.routesService.getOrderDetail(dateApi, exp.expediente_id.toString());
      //   const decrypted = await this.cryptoService.decryptData(response.data.data);
      //   expedienteDetalleLocal = {
      //     ...(JSON.parse(decrypted) as ExpedienteDetalleLocal),
      //   };
      // }

      this.headerService.setSubtitleTwo(`${expedientesLocal.length} Entregas`);
